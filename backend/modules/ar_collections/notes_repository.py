...
class ARCollectionsNotesRepository:
        ...
        with self._initialization_lock:
            ...
            try:
                connection.executescript(
                    """
                    CREATE TABLE IF NOT EXISTS ar_collections_notes (
                    ...
                    ON ar_collections_notes(
                        customer_number, created_at DESC, note_id DESC
                    );

                    CREATE TRIGGER IF NOT EXISTS
                    ar_collections_notes_no_update
                    BEFORE UPDATE ON ar_collections_notes
                    BEGIN
                        SELECT RAISE(
                            ABORT,
                            'AR collections notes are append-only.'
                        );
                    END;

                    CREATE TRIGGER IF NOT EXISTS
                    ar_collections_notes_no_delete
                    BEFORE DELETE ON ar_collections_notes
                    BEGIN
                        SELECT RAISE(
                            ABORT,
                            'AR collections notes are append-only.'
    ...
    def create_note(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            connection.execute(
                """
                INSERT INTO ar_collections_notes (
    ...
    def get_note(self, note_id: str) -> dict[str, Any] | None:
        ...
        try:
            row = connection.execute(
                "SELECT * FROM ar_collections_notes WHERE note_id = ?;",
                (note_id,),
            ).fetchone()
        finally:
            connection.close()
    ...
    def list_notes(self, customer_number: int) -> list[dict[str, Any]]:
        ...
        try:
            rows = connection.execute(
                """
                SELECT * FROM ar_collections_notes
    ...
    def _note_from_row(row: sqlite3.Row) -> dict[str, Any]:
        ...
        if actual_hash != expected_hash:
            raise ARCollectionsNoteIntegrityError(
