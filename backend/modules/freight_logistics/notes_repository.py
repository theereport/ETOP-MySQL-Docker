...
from data.mysql import get_engine, logistics_notes_table, metadata


class RouteNoteIntegrityError(RuntimeError):
    """Raised when a stored logistics note fails its SHA-256 check."""


class RouteNotesRepository:
    """Local append-only ETOP evidence: professional logistics notes."""
    ...
    def initialize(self) -> None:
        with self._initialization_lock:
            metadata.create_all(
                self._engine, checkfirst=True, tables=[logistics_notes_table]
            )
    ...
    def create_note(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._engine.begin() as connection:
            connection.execute(
                logistics_notes_table.insert().values(
    ...
    def _get_note(self, connection, note_id: str) -> dict[str, Any] | None:
        row = connection.execute(
            select(logistics_notes_table).where(
                logistics_notes_table.c.note_id == note_id
            )
        ).mappings().first()
    ...
    def list_notes(self, route_code: str) -> list[dict[str, Any]]:
        ...
        with self._engine.connect() as connection:
            rows = connection.execute(
                select(logistics_notes_table)
                .where(logistics_notes_table.c.route_code == route_code)
                .order_by(
                    logistics_notes_table.c.created_at.desc(),
                    logistics_notes_table.c.note_id.desc(),
                )
            ).mappings().all()
