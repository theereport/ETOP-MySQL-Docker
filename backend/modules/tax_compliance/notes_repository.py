...
from data.mysql import get_engine, metadata, tax_compliance_notes_table
...
class TaxComplianceNotesRepository:
    """Local append-only ETOP evidence: professional tax compliance notes."""
    ...
    def initialize(self) -> None:
        with self._initialization_lock:
            metadata.create_all(
                self._engine,
                checkfirst=True,
                tables=[tax_compliance_notes_table],
            )
    ...
    def create_note(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._engine.begin() as connection:
            connection.execute(
                tax_compliance_notes_table.insert().values(
    ...
    def _get_note(self, connection, note_id: str) -> dict[str, Any] | None:
        row = connection.execute(
            select(tax_compliance_notes_table).where(
                tax_compliance_notes_table.c.note_id == note_id
            )
        ).mappings().first()
    ...
    def list_notes(self, customer_number: int) -> list[dict[str, Any]]:
        ...
        with self._engine.connect() as connection:
            rows = connection.execute(
                select(tax_compliance_notes_table)
                .where(
                    tax_compliance_notes_table.c.customer_number
                    == customer_number
                )
                .order_by(
                    tax_compliance_notes_table.c.created_at.desc(),
                    tax_compliance_notes_table.c.note_id.desc(),
                )
            ).mappings().all()
    ...
    def _note_from_row(row) -> dict[str, Any]:
        ...
        verify_snapshot_hash(
            snapshot_json,
            expected_hash,
            error=TaxComplianceNoteIntegrityError,
            message=(
