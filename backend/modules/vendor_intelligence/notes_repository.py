...
class VendorNoteIntegrityError(RuntimeError):
    """Raised when a stored vendor note fails its SHA-256 integrity check."""


class VendorNotesRepository:
    """Local append-only ETOP evidence: professional notes on a vendor."""
        ...
        with self._initialization_lock:
            ...
            try:
                connection.executescript(
                    ...
                    CREATE INDEX IF NOT EXISTS idx_vendor_notes_vendor_time
                    ON vendor_notes(vendor_number, created_at DESC, note_id DESC);

                    CREATE TRIGGER IF NOT EXISTS vendor_notes_no_update
                    BEFORE UPDATE ON vendor_notes
                    BEGIN
                        SELECT RAISE(ABORT, 'Vendor notes are append-only.');
                    END;

                    CREATE TRIGGER IF NOT EXISTS vendor_notes_no_delete
                    BEFORE DELETE ON vendor_notes
                    BEGIN
                        SELECT RAISE(ABORT, 'Vendor notes are append-only.');
                    END;
                    """
        ...
        try:
            ...
            connection.execute(
                ...
                INSERT INTO vendor_notes (
                    note_id, vendor_number, vendor_name, author_identity,
    ...
    def get_note(self, note_id: str) -> dict[str, Any] | None:
        ...
        try:
            row = connection.execute(
                "SELECT * FROM vendor_notes WHERE note_id = ?;",
                (note_id,),
            ).fetchone()
        finally:
            connection.close()
