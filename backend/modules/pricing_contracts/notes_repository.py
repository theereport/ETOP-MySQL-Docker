...
from data.mysql import get_engine, metadata, pricing_notes_table
...
class PricingNotesRepository:
    ...
    def initialize(self) -> None:
        with self._initialization_lock:
            metadata.create_all(
                self._engine, checkfirst=True, tables=[pricing_notes_table]
            )
    ...
    def create_note(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._engine.begin() as connection:
            connection.execute(
                pricing_notes_table.insert().values(
                    ...
                    matched_discount_count=record["matched_discount_count"],
    ...
    def _get_note(self, connection, note_id: str) -> dict[str, Any] | None:
        row = connection.execute(
            select(pricing_notes_table).where(
                pricing_notes_table.c.note_id == note_id
            )
        ).mappings().first()
    ...
    def list_notes(
        ...
        *,
        ...
        self.initialize()
        conditions = [pricing_notes_table.c.customer_number == customer_number]

        for column, value in (
            (pricing_notes_table.c.vendor_code, vendor_code),
            (pricing_notes_table.c.product_class, product_class),
            (pricing_notes_table.c.product_number, product_number),
            (pricing_notes_table.c.product_type, product_type),
        ):
        ...
        with self._engine.connect() as connection:
            rows = connection.execute(
                select(pricing_notes_table)
                .where(*conditions)
                .order_by(
                    pricing_notes_table.c.created_at.desc(),
                    pricing_notes_table.c.note_id.desc(),
                )
            ).mappings().all()
    ...
    def _note_from_row(row) -> dict[str, Any]:
        ...
        verify_snapshot_hash(
            snapshot_json,
            expected_hash,
            error=PricingNoteIntegrityError,
            message=(
