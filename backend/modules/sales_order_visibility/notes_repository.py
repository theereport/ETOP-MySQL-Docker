...
class OrderNoteIntegrityError(RuntimeError):
    """Raised when a stored order note fails its SHA-256 integrity check."""


class OrderNotesRepository:
    """Local append-only ETOP evidence: professional notes on an invoice."""
        ...
        with self._initialization_lock:
            ...
            try:
                connection.executescript(
                    ...
                    CREATE INDEX IF NOT EXISTS idx_order_notes_invoice_time
                    ON order_notes(invoice_number, created_at DESC, note_id DESC);

                    CREATE TRIGGER IF NOT EXISTS order_notes_no_update
                    BEFORE UPDATE ON order_notes
                    BEGIN
                        SELECT RAISE(ABORT, 'Order notes are append-only.');
                    END;

                    CREATE TRIGGER IF NOT EXISTS order_notes_no_delete
        ...
        try:
            ...
            connection.execute(
                ...
                INSERT INTO order_notes (
                    note_id, invoice_number, customer_number, customer_name,
    ...
    def get_note(self, note_id: str) -> dict[str, Any] | None:
        ...
        try:
            row = connection.execute(
                "SELECT * FROM order_notes WHERE note_id = ?;",
                (note_id,),
            ).fetchone()
        finally:
            connection.close()
